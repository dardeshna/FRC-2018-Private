...
public class Elevator extends Subsystem {
	...
	public static void resetInstance() { instance = new Elevator("Elevator"); }
	...
	public void update(Commands commands, RobotState robotState) {
		...
		switch(mState) {
			...
			case CUSTOM_POSITIONING:
				//Control loop
				mOutput.setPosition(
						kElevatorBottomPosition.get() + mElevatorWantedPosition.get(),
	...
	/**
	 * Process wanted elevator state and joystick inputs into mState for the state machine.
	 * Sets {@link Elevator#mElevatorWantedPosition} for use in the state machine. Does not clear it.
	 * At the end, always check if any custom positioning has finished, and if so, set the state to hold. <br> <br>
	 *
	 * <b>Teleop joystick movement overrides everything else!</b> <br> <br>
	 *
	 ...
	 *         Sets the desired holding position and state to hold.
	 *     </li>
	 *     <li>
	 *         {@link ElevatorState#MANUAL_POSITIONING}:
	 *         Sets the state to manual. If joystick input is within deadband, set to hold instead.
	 *     </li>
	 *     <li>
	 *         {@link ElevatorState#CUSTOM_POSITIONING}:
	 *         Sets the desired custom position and state to custom positioning. If not calibrated, set to calibrate instead.
	 *     </li>
	 *     <li>
	 *         {@link ElevatorState#IDLE}:
	 *         Sets to idle.
	 *     </li>
	 * </ul>
	 *
	...
	private void handleState(Commands commands) {
	    if(commands.wantedElevatorState == ElevatorState.CALIBRATING) {
        	...
        	//Set the wanted elevator position if not already set, or if switching from a different state
	        ...
	        if(kElevatorBottomPosition.isPresent() && kElevatorTopPosition.isPresent()) {
                //Set the setpoint
				//If the desired custom positioning setpoint is different than what currently exists, replace it
	...
	/**
	 * Calibrates the bottom or top position values depending on which HFX is triggered.
	 * If the other position is not already set, set that as well.
	 */
	 ...
	 *
	 * @return <p> false if {@link Elevator#mState} is not {@link ElevatorState#CUSTOM_POSITIONING},
	 * 			   or whether it's within position and velocity tolerances otherwise </p>
	 */
